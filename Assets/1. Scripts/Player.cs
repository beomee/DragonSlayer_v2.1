...
public class Player : MonoBehaviour
{
    ...
    {
        attackCollision4.SetActive(true);  
        AudioManager.instance.PlaySE(PlayerAttack_Sound3, 1,1);
    ...
    private void Update()
    {
        ...
        if (inventoryUi.InvenUiOpen == false && gameSceneMng.isSettingUi == false && storeUi.isShopOpenUi == false && npc_.isNpcUi == false && Json.instance.data.hp > 0.0f )
        {

                // 마우스 왼쪽 클릭 시 공격
                if (Input.GetMouseButtonDown(0))
                {
                    OnweaponAttack();  // 공격 애니메이션 실행   
                }
    ...
    public void UseStamina(float amount) // 스태미나 사용 함수
    {
        if (currentStamina - amount >= 0)  
        {
            ...
            {
                StopCoroutine(RegenStamina());   // 리젠하는것을 멈춤
            }
        }
    }
    ...
    {
        if (regen == null) // 스태미나가 가득 차지 않았을 경우
        {
    ...
    IEnumerator KnockBackMoveCool() // 넉백 시 1초간 경직
    {
        yield return new WaitForSeconds(0.1f);
    ...
    {
        float knockBackSpeed_Basic = 100.0f; // 뒤로 밀리는 힘

        while (knockBackSpeed_Basic > 1.1f) // 제곱근 계산 시 값이 크게 줄어드는 구간 : 1.1f
        {
    ...
    IEnumerator KnockBack_Shield() // 막기
    {
        shieldEffect.SetActive(true); // 파티클 이펙트 생성

        float knockBackSpeed_Shield = 30.0f; 

        while (knockBackSpeed_Shield > 1.1f) 
        {
            Vector3 moveDirection = -transform.forward.normalized;
    ...
    IEnumerator Damaged_Burn()
    {
        ...
        burningDebuff.SetActive(true); // 디버프 Ui On
        ...
        Json.instance.data.hp -= redDragon.enemystr / 10.0f;    
        hpBar.value = Json.instance.data.hp; 
        ...
        burningDebuff.SetActive(false); // 디버프 Ui Off

    }

    IEnumerator Damaged_Burn_Color() // 화상 시 캐릭터 색깔 변경되는 함수 
    {
    ...
    IEnumerator Damaged_Shouting() // 샤우팅 공격에 맞았을 경우
    {
